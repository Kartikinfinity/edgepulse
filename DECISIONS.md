...
**Why.** Measured on this host: **C: has 0 bytes free** (128 GB NVMe, 100 % full). D: and E:
are partitions of a 1 TB SATA HDD with 337 GB and 446 GB free. A Phase-1 build needs a
570 MB dataset expanded to 21,267 WAVs, feature caches, model checkpoints, PlatformIO build
output and an ASR model download — none of which fits. The Desktop folder is also inside
**OneDrive**, so large binary artefacts would be continuously sync-churned.

**Consequences.**
- PlatformIO **core** stays at `C:\Users\Menon\.platformio` (already installed, 2.96 GB —
  reinstalling it elsewhere would need 3 GB we do not have). Only `build_dir` moves to E:.
- The Python venv and `PIP_CACHE_DIR` must be on E:, or `pip install` fails.
...
- Fully reversible: move the folder once C: is freed.

**Recommended to the user:** free space on C: regardless. 0 bytes free also risks Windows
update failures, page-file growth failures, and OneDrive sync errors.
